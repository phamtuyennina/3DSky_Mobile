	...
	function save_item()
	{
		...
		if(isset($id) && $id!=0)
		{
			...
			if($d->update('order',$data)){
				if($_POST['old_tinhtrang']==1 && $data['tinhtrang']==2){
					$rowOrder=$d->rawQueryOne("select * from #_order where id=?",array($id));
					if(!empty($rowOrder['id_user'])){
						$rowUser=$d->rawQueryOne("select numpro,id from #_member where id=?",array($rowOrder['id_user']));
						if(!empty($rowOrder['numfree'])){
			                $dataUser['free_start']=time();
			                $dataUser['free_end']=time()+(86400*$rowOrder['numfree']);
			            }
			            $dataUser['numpro']=$rowUser['numpro']+$dataUser['numpro'];
