<?php
	...
	function save_item()
	{
		...
		if(!empty($data)) { foreach($data as $column => $value){
			if(is_array($value)) $data[$column] = htmlspecialchars(implode(',',$value));
			else $data[$column] = htmlspecialchars($value);
		...
		$data['manufacturers'] = (isset($data['manufacturers'])) ? 1 : 0;
		$data['type'] = $type;
	...
	function get_items_properties()
	{
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select * from #_product_properties where type = ? $where order by stt,id desc $limit";
		$items = $d->rawQuery($sql,array($type));
		$sqlNum = "select count(*) as 'num' from #_product_properties where type = ? $where order by stt,id desc";
		$count = $d->rawQueryOne($sqlNum,array($type));
		$total = $count['num'];
		$url = "index.php?com=product&act=man_properties&type=".$type;
		$paging = $func->pagination($total,$per_page,$curPage,$url);
	}

	/* Edit color */
	function get_item_properties()
	{
		global $d, $func, $curPage, $item, $type;

		$id = (isset($_GET['id'])) ? (int)htmlspecialchars($_GET['id']):0;
		if(empty($id)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage, false);

		$item = $d->rawQueryOne("select * from #_product_properties where id = ?",array($id));

		if(empty($item['id'])) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage, false);
	}

	/* Save color */
	function save_item_properties()
	{
		global $d, $func, $curPage, $config, $type;

		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage, false);
		...
		if(isset($id) && $id!=0)
		{
			if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_properties'], UPLOAD_MAU,$file_name))
			{
				$data['photo'] = $photo;
				$row = $d->rawQueryOne("select id, photo from #_product_properties where id = ? and type = ?",array($id,$type));
				if(!empty($row['id'])) $func->delete_file(UPLOAD_MAU.$row['photo']);
			}
			...
			$d->where('id', $id);
			$d->where('type', $type);
			if($d->update('product_properties',$data)) $func->transfer("Cập nhật dữ liệu thành công", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage);
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage, false);
		}
		else
		{	
			if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_properties'], UPLOAD_MAU,$file_name))
			...
			if($d->insert('product_properties',$data)) $func->transfer("Lưu dữ liệu thành công", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage);
			else {
				$func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage, false);
			}
		}
	}

	/* Delete color */
	function delete_item_properties()
	{
		global $d, $curPage, $func, $type;
		...
		if(isset($id) && $id!=0)
		{
			$row = $d->rawQueryOne("select * from #_product_properties where id = ? and type = ?",array($id,$type));

			if(!empty($row['id']))
			{
				$func->delete_file(UPLOAD_MAU.$row['photo']);
				$d->rawQuery("delete from #_product_properties where id = ?",array($id));

				$func->transfer("Xóa dữ liệu thành công", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage);
			}
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage, false);
		}
		elseif(isset($_GET['listid']))
		{
			$listid = explode(",",$_GET['listid']);
		...
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage, false);
	}
?>