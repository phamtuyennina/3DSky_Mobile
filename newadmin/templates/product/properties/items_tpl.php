<?php 
	$linkMan = "index.php?com=product&act=man_properties&type=".$type."&p=".$curPage;
	$linkAdd = "index.php?com=product&act=add_properties&type=".$type."&p=".$curPage;
    $linkEdit = "index.php?com=product&act=edit_properties&type=".$type."&p=".$curPage;
    $linkDelete = "index.php?com=product&act=delete_properties&type=".$type."&p=".$curPage;
?>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <h5 class="mb-0 text-capitalize">Quản lý thuộc tính</h5>
                        ...
                        <table class="table mb-0 w-100" id="datatable">
                            <thead>
                                <tr class="bg-white">
                                    ...
                                    <?php } ?>
                                    <?php if(!empty($config['product'][$type]['mau_properties']) && $config['product'][$type]['mau_properties']==true) { ?>
                                ...
                                <?php for($i=0;$i<count($items);$i++) { ?>
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <input type="number" class="form-control form-control-mini m-auto update-stt" min="0" value="<?=$items[$i]['stt']?>" data-id="<?=$items[$i]['id']?>" data-table="product_properties">
                                        </td>
                                        <?php if(!empty($config['product'][$type]['mau_images']) && $config['product'][$type]['mau_images']==true) { ?>
                                            <td class="align-middle">
                                                <a href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['tenvi']?>"><img class="rounded img-preview" onerror="src='<?=THUMBS?>/<?=$config['product'][$type]['thumb_properties']?>/assets/images/noimage.png'" src="<?=UPLOAD_MAU.$items[$i]['photo']?>" alt="<?=$items[$i]['tenvi']?>"></a>
                                            </td>
                                        <?php } ?>
                                        <?php if(!empty($config['product'][$type]['mau_properties']) && $config['product'][$type]['mau_properties']==true) { ?>
                                            <td >
                                                <input type="color" readonly disabled class="form-control" value="#<?=$items[$i]['mau']?>">
                                            </td>
                                        <?php } ?>
                                        <td class="align-middle" >
                                        ...
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox form-switch">
                                                <input type="checkbox" class="form-check-input show-checkbox" id="show-checkbox-<?=$items[$i]['id']?>" data-table="product_properties" data-id="<?=$items[$i]['id']?>" data-loai="hienthi" <?=($items[$i]['hienthi'])?'checked':''?>>
